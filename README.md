...
<p align="center">
  <strong>全部 SSH Host 均可识别（历史模式一次启用一个）</strong><br>
...
7. 安装器准备远端 Codex 运行文件与共享历史助手，并逐个 Host 执行 doctor，直到全部显示 `READY`。
...
> [!WARNING]
> **同一集群账号一次只连接一个计算节点。** JumpBridge 在安装阶段把旧对话合并到
> 专用主历史，连接后由当前 Host 直接使用；若另一个 Host 已占用历史锁，新连接会
> 明确失败，避免相同 thread ID 被重复加载。切换节点前先断开当前连接，确认离线后
> 再启用目标节点。远端 `~/.codex` 和已有对话不会因此被删除。
...
[OK] Remote shared history coordination is ready
...
- **Windows 10/11：**下载并双击 `Codex-JumpBridge-Windows-v1.4.1.exe`。
- **macOS 11+：**下载 `Codex-JumpBridge-macOS-v1.4.1.dmg`，将 App 拖入“应用程序”后运行。
...
VS Code/Cursor 继续使用远端 `~/.codex`，JumpBridge 只在安装或显式准备历史时把它作为
只读导入源。三个计算节点按顺序使用 JumpBridge 专用主历史；互斥锁保证同一账号一次只有
一个 Host 的 app-server 读写这份 SQLite，因此重连时无需再遍历、复制上千个共享存储文件。
app-server 的 socket/PID/log 仍位于节点本地短路径，不会改写 VS Code/Cursor 或本机 Codex 历史。
...
这是历史互斥保护，不是节点损坏。同一账号一次只启用一个 Host；切换时先关闭当前连接，
确认离线后再启用目标 Host。若所有 Host 都失败，再运行 doctor 检查 SSH、远端运行文件和代理。
