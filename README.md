...
> [!WARNING]
> **同一集群账号建议一次只连接一个计算节点。** 多个节点通常共享同一份远端
> `~/.codex` 历史，同时启用多个 SSH 连接会让相同的 thread ID 被不同 Host 重复加载，
> 可能出现频道跳到其他项目、重启后项目显示“无任务”等现象。切换节点前请先断开
> 当前连接，确认其变为离线后再启用目标节点；这些显示问题不代表远端对话被删除。
