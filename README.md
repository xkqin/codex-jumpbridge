...
7. 安装器准备远端 Codex 运行文件与历史隔离助手，并逐个 Host 执行 doctor，直到全部显示 `READY`。
...
> [!WARNING]
> **同一集群账号一次只连接一个计算节点。** JumpBridge 会在连接时把共享历史同步到
> 当前 Host 的独立运行目录，并在断开时回写到 JumpBridge 主历史；若另一个 Host 已占用历史锁，新连接会
...
VS Code/Cursor 继续使用远端 `~/.codex`，JumpBridge 只把它作为只读导入源。启动时会把
其中的 rollout 对话记录合并到远端主历史，再同步给当前 Host；断开时先停止该 Host 的
app-server，再把新增记录写回 JumpBridge 主历史。每个 Host 保留独立的 `state_5.sqlite`，
app-server 的 socket/PID/log 位于节点本地短路径，不会同时读写一个 SQLite，也不会改写
VS Code/Cursor 或本机 Codex 历史。
